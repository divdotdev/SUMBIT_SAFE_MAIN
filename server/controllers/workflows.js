...
export function workflowsController(store, config, providers) {
  return {
    ...
    createApplication: async (req, res) => {
      ...
      const readiness = await overallReadiness(store, req.user._id, { loanType: loan.loanType, product: loan });
      let application;
      for (let attempt = 0; attempt < 10; attempt += 1) {
        try {
          application = await store.create('LoanApplication', { ...req.body, userId: req.user._id, status: 'Draft', readinessScore: readiness.overallScore,
            applicationCode: `SS-${loan.loanType}-${new Date().getFullYear()}-${randomInt(10000, 100000)}`, dataMode: loan.dataMode });
          break;
    ...
    applicationStatus: async (req, res) => {
      ...
      const loan = await store.one('LoanProduct', { _id: application.loanProductId });
      if (!loan) throw new AppError(404, 'Loan product not found');
      const readiness = await overallReadiness(store, req.user._id, { loanType: loan.loanType, product: loan });
