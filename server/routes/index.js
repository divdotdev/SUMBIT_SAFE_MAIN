...
export function createRoutes(store, config, providers) {
  ...
  router.post('/applications', secured, validate(z.object({ loanProductId: id, loanType: z.enum(['HOME', 'EDUCATION']), loanAmount: z.number().positive().max(1e10), tenure: z.number().int().min(1).max(40), consentId: id.optional() }).strict()), w.createApplication);
