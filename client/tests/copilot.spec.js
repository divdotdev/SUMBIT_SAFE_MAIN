...
test('Copilot no context, unavailable provider, thinking and retryable error states', async ({ page }) => {
  ...
  const panel = page.getByRole('complementary', { name: 'SUBMIT SARTHI' });
  await expect(panel).toContainText('Select a loan type, product or application');
  await panel.getByRole('button', { name: 'Close Submit Sarthi' }).click();
  ...
  await expect(panel.getByRole('status')).toContainText('Submit Sarthi is thinking');
  release();
  await expect(panel.getByRole('region', { name: 'Submit Sarthi response' })).toContainText('Provider error');
  await page.unroute('**/copilot/chat');
  await panel.getByRole('button', { name: 'What should I fix next?', exact: true }).click();
  await expect(panel.getByRole('region', { name: 'Submit Sarthi response' })).toContainText('Upload AADHAAR');
  await panel.getByLabel('Ask about this application').fill('Am I approved?');
  await panel.getByRole('button', { name: 'Ask Submit Sarthi', exact: true }).click();
  await expect(panel.getByRole('region', { name: 'Submit Sarthi response' })).toContainText('unavailable');
  await page.keyboard.press('Escape');
  await expect(panel).not.toBeVisible();
  await expect(page.getByRole('button', { name: 'SUBMIT SARTHI', exact: true })).toBeFocused();
});
